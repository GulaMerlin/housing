...
class User(BaseModel, db.Model,UserMixin ):
    ...
    id_cart = db.Column(db.String(18), unique=True) # 实名责任生份证号码
    email = db.Column(db.String(20), unique=True)
    house=db.relationship('House', backref='user')
...
class House(BaseModel, db.Model):
    ...
    unit = db.Column(db.String(32), default='') # 房间单元，几室几厅
    capacity = db.Column(db.Integer, default=1) # 房间容纳人数
    beds = db.Column(db.String(64), default='') # 房屋床铺配置
    deposit = db.Column(db.Integer, default=0) # 押金
    min_days = db.Column(db.Integer, default=1) # 最少入住天数
    max_days = db.Column(db.Integer, default=0) # 最多入住天数 0不限制
